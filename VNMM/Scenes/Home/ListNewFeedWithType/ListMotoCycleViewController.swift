//
//  ListMotoCycleViewController.swift
//  VNMM
//
...
class ListMotoCycleViewController: UIViewController, NavigationControllerCustomDelegate {

    @IBOutlet weak var col_listMotoCycle: UICollectionView!
    
    
    var listNewfeed = [NewFeed]()
    var page = 1
    var limit = 100
    var typeCar = POST_TYPE.MOTOCYCLE.rawValue
    var prioritize_type = -1
    
    override func viewDidLoad() {
        super.viewDidLoad()

        col_listMotoCycle.register(UINib(nibName: "NewFeedItemTableViewCell", bundle: nil), forCellWithReuseIdentifier: "NewFeedItemCollectionViewCell")
        
        col_listMotoCycle.delegate = self
        col_listMotoCycle.dataSource = self
                ...
                if(data != nil) {
                    ...
                    if response.code == 200 {
                        if let DataResponse  = Mapper<NewFeedResponse>().map(JSONObject: response.data) {
                            ...
                            self.col_listMotoCycle.reloadData()
                        }
...
extension ListMotoCycleViewController : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
...
//        cell.view_item.layer.shadowOffset = .zero
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: col_listMotoCycle.bounds.width, height: 400)
    }
...
extension ListMotoCycleViewController : DetailNewfeedDelegate {
