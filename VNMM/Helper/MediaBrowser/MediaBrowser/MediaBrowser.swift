...
@objcMembers public class MediaBrowser: UIViewController, AVPlayerViewControllerDelegate {
    ...
    internal var mediaArray = [MediaGitHub?]()
    internal var thumbMedias = [MediaGitHub?]()
    /// Provided via init
	internal var fixedMediasArray: [MediaGitHub]?
     ...
     */
    public convenience init(media: [MediaGitHub]) {
        self.init()
...
//        delegateSave?.saveImage(indexPath: indexPath, initIndex: 0, imageDisplay: self.img_1)
//        delegateBrowserMedia?.mediaDid(selected: <#T##Bool#>, at: <#T##Int#>, in: <#T##MediaBrowser#>)

        alertController = UIAlertController()
//        alertController = UIAlertController(title: "", message: "", preferredStyle: .actionSheet)
        let saveAction = UIAlertAction(title: "Lưu hình ảnh", style: .default) { [self](action) in


           
            

//            let imagedisplay = mediaAtIndex(index: currentIndex)
//            guard let image = imagedisplay?.placeholderImage else { return }
        ...
        if let navi = navigationController {
            if navi.viewControllers.count > 0 && navi.viewControllers[0] == self {
                ...
                doneButton = UIBarButtonItem(
                    title: NSLocalizedString("Xong", comment: ""),
                    style: .done,
                    target: self,
...
//                    newBackButton.setTitleTextAttributes([String : AnyObject](), for: .highlighted)
    ...
    func mediaAtIndex(index: Int) -> MediaGitHub? {
        var media: MediaGitHub? = nil
    ...
    func thumbPhotoAtIndex(index: Int) -> MediaGitHub? {
        var media: MediaGitHub?
    ...
    func image(for media: MediaGitHub?) -> UIImage? {
        if let p = media {
    ...
    func loadAdjacentPhotosIfNecessary(photo: MediaGitHub) {
    ...
    @objc func handlePhotoLoadingDidEndNotification(notification: NSNotification) {
        if let photo = notification.object as? MediaGitHub {
