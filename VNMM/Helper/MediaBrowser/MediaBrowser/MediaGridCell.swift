...
class MediaGridCell: UICollectionViewCell {
    ...
    let loadingIndicator = UICircularProgressRing(frame: CGRect(x: 140, y: 30, width: 40, height: 40))
    ...
    @objc func setProgressFromNotification(notification: NSNotification) {
        DispatchQueue.main.async() {
            ...
            if let photoWithProgress = dict["photo"] as? MediaGitHub, let progress = dict["progress"] as? CGFloat, let p = self.photo, photoWithProgress.equals(photo: p) {
    ...
    @objc func handlePhotoLoadingDidEndNotification(notification: NSNotification) {
        if let p = notification.object as? MediaGitHub, let mwp = Media, photosEqual(p1: p, mwp) {
    ...
    func photosEqual(p1: MediaGitHub, _ p2: MediaGitHub) -> Bool {
        return
